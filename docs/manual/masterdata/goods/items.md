...
Справочник доступен в меню **Справочники - Товары -  Товары** (рис. 1).
...
- **Со сроком годности** - если установлено, то при вводе товара, будет запрашиваться либо срок годности, либо дату производства. 
Это зависит от настроек вкладки **Срок годности**, которая активируется если параметр выбран
...
Распечатать этикетку можно из двух мест:
- **Справочники-Товары** с выбором перед печатью шаблона этикетки из списка, закрепленных за товарной категорией (флажок "Вкл.") этикеток, по текущей отдельной позиции
- **Печать-Товар-Печать этикеток товара** печать этикетки по умолчанию (флажок "Умол") для всех отмеченных или только текущей товарной позиции    

**См. также**
- Для загрузки новых шаблонов, см. Печать-Товар-Шаблон этикеток товара 
- Для установки форматы этикетки, см. Печать-Принтеры-Форматы бумаги
...
2. Ячейки хранения справа (Рис. 9(2)). Чекбокс "Запрет" (Рис. 9(4)) устанавливает фильтр для ячеек, которые помечены, как запрещенные. 
<div className="shifted-text">
::::tip Логика работы
...
Такой подход позволяет быстро организовать работу по приемке товара, и по мере развития системы, впоследствии отсекать ненужные ячейки.
::::
</div>
...
При приемке товара будет запрошена дату производства, если установлен чекбокс "Указывать дату производства". 
